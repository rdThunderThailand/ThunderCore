...
export function DeleteConfirmModal({ onConfirm, onClose, isSubmitting }: {
    onConfirm: () => void;
    onClose: () => void;
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg animate-in zoom-in-95 duration-200 border border-slate-100 overflow-hidden">
                <div className="p-6 flex gap-4">
                    ...
                    <div className="space-y-2">
                        <h3 className="text-lg font-bold text-slate-900">{t('modal.removeMember')}</h3>
                        <p className="text-sm text-slate-500 leading-relaxed">
                            {t('modal.removeMemberDesc')}
                        </p>
                    </div>
                </div>
                <div className="bg-slate-50/50 px-6 py-4 flex justify-end gap-3 border-t border-slate-50">
                    ...
                    >
                        {isSubmitting ? <Loader2 className="w-3 h-3 animate-spin" /> : <Trash2 className="w-4 h-4" />}
                        {t('modal.remove')}
                    </button>
                </div>
            </div>
